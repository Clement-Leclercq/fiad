...
class Particle_Filter:
    ...
    def motion_prediction(self):
        ...
        for i in range(self.NB_PARTICLES) :
            ...
            mu = 0
            sigma = 1
            deplacement_x = 1+ self.increment + int(random.gauss(mu, sigma))
            sigma = 2
            deplacement_y = self.y_pos + int(random.gauss(mu, sigma))
            new_particle_list.append(Particle(x_coord+deplacement_x,y_coord+deplacement_y,1,1/self.NB_PARTICLES))
